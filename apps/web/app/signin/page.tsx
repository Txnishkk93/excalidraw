...
export default function SigninPage() {
  ...
  const handleSignin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiFetch("/api/v1/signin", {
        method: "POST",
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-4">
      <div className="w-full max-w-md radius-card border border-[var(--border-color)] bg-[var(--bg-color)] p-8 shadow-sm">
        <h1 className="mb-2 text-2xl font-semibold tracking-tight text-[var(--text-color)]">Welcome back</h1>
        <p className="mb-8 text-sm text-[var(--muted-color)]">Sign in to your NOVA workspace.</p>
        ...
        <p className="mt-6 text-center text-sm text-[var(--muted-color)]">
          ...
          <a href="/signup" className="text-[var(--color-nova-ink)] hover:underline">
            Create one
          </a>
        </p>
      </div>
